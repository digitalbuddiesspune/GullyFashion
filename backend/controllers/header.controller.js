...
export const searchProducts = async (req, res) => {
  try {
    ...
    // Build search conditions - match products containing any of the keywords
    ...
    keywords.forEach(keyword => {
      searchConditions.push(
        ...
        { 'product_info.brand': { $regex: keyword, $options: 'i' } },
        { 'product_info.manufacturer': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtSize': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtMaterial': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtFit': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtSleeve': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtNeck': { $regex: keyword, $options: 'i' } },
        { 'product_info.tshirtColor': { $regex: keyword, $options: 'i' } }
      );
    });
    
    // Also search for the full query string for exact matches
    searchConditions.push(
    ...
    // Search in products - match products that contain any keyword
    const products = await Product.find({
      $or: searchConditions
    })
    .limit(20)
    .select('title images price mrp discountPercent category product_info')
    .sort({ title: 1 }); // Sort alphabetically for consistent results
