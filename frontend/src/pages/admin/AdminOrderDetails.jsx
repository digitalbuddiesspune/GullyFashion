...
const AdminOrderDetails = () => {
  ...
  const normalizedPaymentMethod = useMemo(() => {
    if (!order) return '';
    return (order.paymentMethod || (order.airpayTransactionId ? 'airpay' : (order.razorpayPaymentId ? 'razorpay' : 'cod'))).toUpperCase();
  }, [order]);
  ...
  const transactionId = order?.transactionId || order?.airpayTransactionId || order?.airpayOrderId || order?.razorpayPaymentId || order?.razorpayOrderId || '—';
